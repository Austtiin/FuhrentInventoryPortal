...
import { authConfig, googleEndpoints } from '@/lib/auth/config';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tokenResponse = await fetch(googleEndpoints.token, {
      method: 'POST',
      headers: {
    ...
    const userResponse = await fetch(`${googleEndpoints.userInfo}?access_token=${tokenData.access_token}`);
