...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const endpoint = `GrabInventoryVH${queryString ? `?${queryString}` : ''}`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        error: true,
        message: `Failed to fetch vehicle inventory data: ${response.statusText}`,
    ...
    const responseTimeMs = Date.now() - startTime;
    console.error('❌ GrabInventoryVH API error:', error);
    
    return NextResponse.json({
      error: true,
      message: 'Failed to fetch vehicle inventory data',
