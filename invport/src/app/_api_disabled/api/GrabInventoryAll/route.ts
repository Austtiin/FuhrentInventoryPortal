...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const endpoint = `GrabInventoryAll${queryString ? `?${queryString}` : ''}`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        error: true,
        message: `Failed to fetch inventory data: ${response.statusText}`,
    ...
    const responseTimeMs = Date.now() - startTime;
    console.error('❌ GrabInventoryAll API error:', error);
    
    return NextResponse.json({
      error: true,
      message: 'Failed to fetch all inventory data',
